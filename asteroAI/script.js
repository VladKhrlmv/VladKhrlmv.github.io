...
if (AUTOMATION_ON) {
    ...
    for (let i = 0; i < NUM_SAMPLES; i++) {
        ...
        let vx =
            ((Math.random() * ROIDS_SPEED * (Math.random() * 40 + 10)) / FPS) *
            (Math.random() < 0.5 ? 1 : -1);
        let vy =
            ((Math.random() * ROIDS_SPEED * (Math.random() * 40 + 10)) / FPS) *
            (Math.random() < 0.5 ? 1 : -1);
...
function angleToPoint(x, y, bearing, targetX, targetY) {
    let angleToTargert = Math.atan2(-targetY + y, targetX - x);
    let diff = bearing - angleToTargert;
...
function update() {
    ...
    if (AUTOMATION_ON) {
        ...
        let vx = roids[c].xv;
        let vy = roids[c].yv;
        let predict = nn.feedForward(normaliseInput(ax, ay, angle, sa, vx, vy))
            .data[0][0];
